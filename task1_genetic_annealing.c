...
int main(int argc,char**argv){
    ...
    //選択の際の焼きなましの温度
    //T0とT1を時間によって変える
    float T0_0=100,T0_1=0.01;
    float T1_0=0.01,T1_1=0.00001;
    ...
    while((double)(end-start)/CLOCKS_PER_SEC*1000<=TL){
        ...
        //焼きなましのパラメータ
        float t=(double)(end-start)/CLOCKS_PER_SEC*1000/TL;
        float T0=powf(T0_0,1.0-t)*powf(T0_1,t);
        float T1=powf(T1_0,1.0-t)*powf(T1_1,t);
        float T;
        for(int m=0;m<M;++m){
            //a: 交叉を行い2子個体を生成する
            int l,r;
            ...
            //選択で焼き鈍す
            T=powf(T0,(float)(1-m/100.0))*powf(T1,(float)m/100.0);
            ...
            if(score_c1<score_g1){
                ...
                double rnd=rand()/(double)RAND_MAX;
                if(rnd<=exp((score_g1-score_c1)/T)){
                    for(int i=0;i<n_weights;++i)g1[i]=c1[i];
                    Scores[cur1]=score_c1;
                }
            }
